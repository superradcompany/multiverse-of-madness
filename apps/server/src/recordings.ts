...
type Record = { world: WorldView; at: number; frame: string };
...
type Entry = { index: Index; pending: Record[] };

// Each segment contains genuine frames and matching engine observations, not
// executable snapshots. At most one second of buffered frames can be lost on crash.
export class Recordings {
  private worlds = new Map<string, Entry>();
  private bytes = 0;
  private chain = Promise.resolve();
  error?: string;
  private protectedIds = new Set<string>();
  private collected = 0;
  constructor(private root: string, private limit = 1024 ** 3,
  ...
  async record(world: WorldView, frame: Buffer) {
    return this.enqueue(async () => {
      if (this.error) return;
      let entry = this.worlds.get(world.id);
      if (!entry) {
        entry = { index: { id: world.id, label: world.label, parentId: world.parentId, startTick: world.state.tick, firstFrame: 0, nextSegment: 0, segments: [], ticks: [], times: [] }, pending: [] };
        this.worlds.set(world.id, entry);
      }
      if (entry.index.ticks.at(-1) === world.state.tick) return;
      const at = Date.now();
      entry.pending.push({ world: structuredClone(world), at, frame: frame.toString('base64') });
      entry.index.ticks.push(world.state.tick); entry.index.times.push(at);
      if (entry.pending.length >= 35) await this.flushEntry(entry);
    });
  }
  ...
  private async flushEntry(entry: Entry) {
  ...
  private async buildPath(endpointId: string, untilTick?: number, single = false): Promise<ReplayPath> {
    ...
    const path: Entry[] = [], seen = new Set<string>();
    ...
    for (const entry of path) {
      ...
      if (start === undefined) {
        if (!(index.firstFrame ?? 0)) start = index.ticks[0];
          ...
          if (record.world.role === 'experiment' && record.world.trial) start = record.world.state.tick - record.world.trial.elapsed;
        }
      }
  ...
  async get(id: string, frameIndex: number): Promise<Record> {
    const result = this.chain.then(() => this.read(id, frameIndex));
    this.chain = result.then(() => {}, () => {});
    return result;
  }
  private async read(id: string, frameIndex: number): Promise<Record> {
    const entry = this.worlds.get(id);
    ...
    for (const segment of entry.index.segments) {
      if (remaining < segment.count) {
        const data = JSON.parse((await decompress(await readFile(join(this.directory(id), segment.file)))).toString()) as Record[];
