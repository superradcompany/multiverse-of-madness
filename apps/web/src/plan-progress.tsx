...
export function PlanProgress({ plan }: { plan?: PlanView }) {
  if (!plan) return null;
  const status = plan.status === 'running' ? `step ${plan.step + 1}/${plan.steps.length}` : plan.status === 'horizon' ? 'time limit' : plan.status === 'replan' ? (plan.reason === 'player died' ? 'ended' : 'needs new plan') : 'complete';
  return <div className="plan-status" title={`${plan.steps.map((step, i) => `${i + 1}. ${step}`).join(' → ')}${plan.reason ? ` · ${plan.reason}` : ''}`}>
    <span className="plan-step-count">{status}</span><span>{plan.status === 'running' ? plan.steps[plan.step] : plan.reason}</span>
