...
export async function openChessLearningHost(directory: string, decision: ChessJevModel, runtime: ChessRuntimeStore) {
  ...
  const fields = { executor: base.revision, adapter: adapter.version, model: contentRevision('chess-prepared-jev', { abi: 'chess-preparation/1', decision: decision.version }), policy: defaultChessPolicy, prompts: {}, skills: [] };
