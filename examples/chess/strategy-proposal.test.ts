...
import { BudgetLedger, type ExecutableSource, type SupervisorProvider } from '@multiverse/gameplay-harness';
  ...
  try {
    ...
    const provider: SupervisorProvider<ChessPolicy, ChessStrategyEvidence> = {
      version: { id: 'fixture', version: '1' }, async propose(request) {
        ...
        return { draft: { reason: 'A strategy for testing', source: proposed }, receipt: { id: request.id, provider: this.version, startedAt: 0, elapsedMs: 1, inputBytes: 1, outputBytes: 1, status: 'complete', requestedModel: 'fixture', servingModels: ['fixture'], usage: { inputTokens: 30, outputTokens: 20 } } };
      },
    };
