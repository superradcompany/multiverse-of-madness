...
import type { ChessWebView } from '../../web-controller.ts';
    ...
    {!view || !main ? <p role="status">Connecting to chess…</p> : <>
      ...
      <div className="layout"><main className="chess-stage">
        ...
        {view.learning && <section className="chess-learning"><div className="learning-heading"><h2><Brain aria-hidden="true" /> Background learning</h2><label><input type="checkbox" aria-label="Enable background learning" checked={view.learning.enabled} disabled={pending} onChange={e => void command({ type: 'learning', enabled: e.target.checked })} />{view.learning.enabled ? 'On' : 'Off'}</label></div>
          ...
          {view.learning.comparison && <button className="watch-comparison" onClick={() => setComparisonOpen(true)}><History aria-hidden="true" />{view.learning.comparison.kind === 'training' ? view.learning.comparison.status === 'running' ? 'Watch practice' : 'Replay practice' : view.learning.comparison.status === 'running' ? 'Watch comparison' : 'Replay comparison'}</button>}
        ...
        {main.temporaryGoal && <section><details open><summary>Temporary goal <span>{main.temporaryGoal.record.status}</span></summary>
          <p>{main.temporaryGoal.record.draft.instruction}</p>
          <p className="hint">{main.temporaryGoal.record.status === 'active'
            ? `${Math.max(0, main.temporaryGoal.record.expiresAt - main.state.ply)} half-moves remaining · temporary guidance`
            : main.temporaryGoal.record.outcome}</p>
          <details><summary>Why this goal</summary><p>{main.temporaryGoal.record.draft.reason}</p></details>
        </details></section>}
