...
export class DoomPreparedModel implements DecisionMaker {
  ...
  async decide(state: GameState, objective: string, history: GameState[], signal: AbortSignal,
    ...
    const policy = context.policy ?? this.revision.policy;
    const input = await doomPreparationInput(this.revision, state, objective, history, experience, actionTicks, { ...context, policy });
    const artifact = await this.options.store.get(this.revision.executor), contextEnded = performance.now();
    let execution: { executorRunId: string; executorElapsedMs: number } | undefined;
    const prepared = await executeLearningStage(input, {
      stage: 'doom-preparation', owner: this.revision.revision.version, artifact,
      executor: this.options.executor, ledger: this.options.ledger, limits: this.limits,
      record: record => {
        execution = { executorRunId: record.receipt.runtime.id, executorElapsedMs: record.receipt.elapsedMs };
        return this.options.record({ revision: this.revision, input: record.input, output: record.output, receipt: record.receipt });
      },
      validate: prepareDoomContext,
    }, signal);
    const preparationEnded = performance.now();
    ...
    return { ...decision, temporaryGoal, latencyMs: ended - started,
      timings: { contextMs: contextEnded - started, preparationMs: preparationEnded - contextEnded,
        judgmentMs: ended - preparationEnded, ...execution! },
