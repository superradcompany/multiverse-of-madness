import { defaultStallForkSeconds } from '../../contracts/src/session.ts';
...
export function doomSupervisorEvidence(saved: SessionCheckpoint, compact = false) {
  ...
  return JSON.parse(JSON.stringify({
    ...
    routing: { stallForkSeconds: saved.view.stallForkSeconds ?? defaultStallForkSeconds, forkThreshold: saved.view.forkThreshold ?? .75 },
...
const sharedTaskParts = [
  ...
  'Optional policy.stallForkSeconds (integer 1–120; default 10) forces alternatives at the next decision after that many selected-path game seconds without observed progress, regardless of confidence. It does not interrupt plans or change progress, recovery or acceptance rules. Balance loop recovery against simulation cost; user overrides win.',
  'Optional policy.execution configures plan interruption and interaction cadence: damageBeforeReplan (health lost since plan start, default 8), nearbyThreatDistance (world units for newly observed threats, default 160), blockedAfterTicks (minimum movement-step age before a stationary-history check can replan, default 35), and usePulseTicks (press/release period, default 7). Those four fields are required when supplied. Optional navigationAlignmentDegrees (1–20, default 7) controls non-combat facing and interaction alignment; movementAlignmentDegrees (1–90, default 25) controls when a movement step turns instead of advancing. Smaller angles demand more alignment and can increase turning time; justify them from route execution evidence. Combat facing still uses motor.aimToleranceDegrees. The clock is 35 ticks/second. Values are bounded by the schema and pinned to each decision. Use observed execution failures to justify changes; these settings cannot alter collision rules, actor identity, legal inputs or independent acceptance metrics. Omission uses the historical defaults.',
  'Optional policy.motor configures movement and aim assistance: stalledTicks (failed movement ticks before escape, default 7), escapeTicks (maximum recovery-heading commitment, 70), escapeDistance (world units before leaving recovery, 64), minimumClearance (player-footprint clearance, 20), lookaheadTicks (velocity lookahead, 4), routeClearance (preferred clearance for an escape direction, 96), aimToleranceDegrees (shared combat-plan alignment and motor fire gate, 6) and turnToTargetDegrees (turn-assistance search angle, 80). Supply all eight within the schema bounds. These preferences run in both live play and independent comparisons under the recorded decision policy. They do not replace geometry, input legality, visibility checks or actor matching. Interaction pulses also honor policy.execution.usePulseTicks during movement. Omitted motor settings preserve historical behavior. Justify adjustments with observed interventions and outcomes rather than assuming wider aim or faster turning is always better.',
...
const taskParts = [
  'Planner output abi=doom-preparation/3 additionally accepts equip steps {kind:"equip",weapon,target:{kind:"point",x,y,z},label,maxTicks}. weapon must be an observed state.weapons entry and state.weaponSelection must be true; fist is not selectable while owning chainsaw because berserk is unobserved. Equip uses real key inputs, waits for observed completion, and stops at its time limit. Choose useful ammo-compatible weapons; ammo order is bullets/shells/cells/rockets. Input weaponPlans supplies equipment alternatives separately from defaultPlans so older /1 and /2 programs remain valid. Combine or replace these plans within the same 10-plan limit; never use equip with /1 or /2. An absent weaponSelection means the detached bridge needs an explicit upgrade/restart before weapon control is available. The basic single-action menu remains unchanged; use conditional plans for equipment changes.',
