...
export function DecisionPanel({ session }: { session: SessionView }) {
  ...
  return <details className={`decision-panel ${branching ? 'split' : 'direct'}`} aria-label="Why this decision">
    <summary className="decision-summary"><div className="decision-heading"><span>{session.stage === 'deciding' ? 'previous decision' : 'why this choice'}</span><strong>{branching ? `${tested} futures` : '1 world'}</strong></div>
    ...
    <details key={`${decision.sourceId}-${decision.tick}`}>
      ...
      <p className="decision-note">Tick {decision.tick} · decision {Math.round(decision.latencyMs ?? 0)} ms{decision.waitMs !== undefined && ` · gameplay waited ${Math.round(decision.waitMs)} ms`}{decision.prefetched && ' · prepared during play'}</p>
    </details>
