...
import type { LearningView } from '../../contracts/src/learning.ts';
...
export function SupervisorActivity({ session }: { session: SessionView }) {
  ...
  return <div className="supervisor-activity">
    ...
    {comparison && proposal && <details className="supervisor-test-details"><summary>See the comparison games</summary><p>These are separate test games, not your main session. Active runs show saved previews. Finished runs with recordings can be replayed.</p><LearningEvaluation key={proposal.id} proposalId={proposal.id} /></details>}
