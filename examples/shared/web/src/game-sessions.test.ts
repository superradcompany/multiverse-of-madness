...
test('local defaults retain the accessed hostname and active selection preserves session paths and queries', () => {
  const sessions = localGameSessions('http://127.0.0.1:4321/chess.html');
  assert.equal(sessions.length, 2);
  assert.equal(sessions[0]!.url, 'http://127.0.0.1:4317/');
  assert.equal(sessions[1]!.url, 'http://127.0.0.1:4321/');
  assert.equal(sameSessionUrl(sessions[1]!.url, 'http://127.0.0.1:4321/chess.html'), true);
  assert.equal(sameSessionUrl(sessions[0]!.url, sessions[1]!.url), false);
