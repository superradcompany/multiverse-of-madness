...
- `apps/web`: React viewer, live previews, commentary, controls, and direction input.
- `apps/server`: authoritative session lifecycle, Jev decisions, scoring, persistence.
- `packages/contracts`: shared TypeScript state and input contracts.
- `packages/game-bridge`: TypeScript HTTP bridge around the freestanding Doom WASM.
