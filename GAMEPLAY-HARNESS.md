...
| Plan completion and interruption | **Revision policy:** optional `execution` sets damage before replanning (default 8 health), new-threat distance (160 world units), blocked-movement check start (35 ticks), and interaction pulse period (7 ticks). **Code:** alignment, arrival, target outcome/loss, ammo exhaustion, death/map change, obstruction checks and step deadlines. | Too eager: repeated judgments and abandoned goals. Too slow: unsafe commitment or wall-sticking. Settings are pinned to the decision, retained in replay/checkpoint policy records and independently evaluated before activation. |
| Motor assistance | **Revision policy:** optional `motor` configures observed-stall delay, recovery duration/distance, clearance/lookahead preferences and aim/turn-assistance angles. **Code:** `doom-controls.ts` / `doom-navigation.ts` enforce geometry, melee reach, target eligibility and per-world recovery; human inputs bypass them. | Live play and independent comparisons use the same recorded policy. Motor interventions can change requested inputs; compare the resulting behavior rather than treating settings alone as improvement. |
...
| Stalled-progress comparison | **Revision policy:** optional `stallForkSeconds`, integer 1–120, default 10. Force alternatives at the next decision after that much selected-route game time without recorded progress, even at high confidence. Explicit user settings win. | Escape confident loops without interrupting active plans. The Doom adapter still owns the meaning of progress; evaluation rules are unchanged. |
...
| --- | --- | --- |
| Engage an enemy | Plausible unblocked target, usable equipped weapon | Face, approach if needed, attack; melee attacks replan when the tracked target leaves conservative reach. Target tracking is approximate without stable actor IDs. |
